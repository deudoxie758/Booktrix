...
export function PublicHeader({ signedIn = false }: { signedIn?: boolean }) {
	return <header className="sticky top-0 z-50 border-b border-sand-200/80 bg-cream-50/90 backdrop-blur-xl">
		<div className="mx-auto flex min-h-18 max-w-7xl items-center justify-between gap-6 px-5 sm:px-8">
			...
			<Link href={signedIn ? '/dashboard' : '/auth/sign-in'} className="rounded-full bg-cocoa-900 px-5 py-2.5 text-sm font-semibold text-white transition hover:bg-cocoa-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-clay-500 focus-visible:ring-offset-2">{signedIn ? 'My account' : 'Sign in'}</Link>
		</div>
	</header>
}
