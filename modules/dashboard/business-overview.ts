...
function operationalAlertsQueries(context: BusinessOverviewContext, db: any, locationIds: string[]) {
...
async function loadOperationsFacts(context: BusinessOverviewContext, db: any, locationIds: string[], start: Date, end: Date): Promise<Partial<OverviewFacts>> {
  const dailyActive = { locationId: { in: locationIds }, startsAt: { gte: start, lt: end }, status: { in: ACTIVE_SEGMENT_STATUSES } }
  const [todayAppointments, pendingApprovals, staffScheduledToday, agendaRecords, groupedCapacity, missingHours, unassignedRequestedBookings, offerings] = await Promise.all([
    ...
    ...operationalAlertsQueries(context, db, locationIds),
  ])
...
export async function loadBusinessOverviewFacts(context: BusinessOverviewContext, now: Date, db: any = prisma): Promise<OverviewFacts> {
  ...
  return { nextAssignedSegment: null, upcomingTimeOff: [], financeOrders: [], ...(await loadOperationsFacts(context, db, locationIds, start, end)) } as OverviewFacts
}
...
function alertsFrom(facts: OverviewFacts): OverviewAlert[] { return [...facts.missingHours.map((location) => ({ kind: 'MISSING_HOURS' as const, message: `${location.name} is missing opening hours.` })), ...(facts.unassignedRequestedBookings ? [{ kind: 'UNASSIGNED_BOOKING' as const, message: `${facts.unassignedRequestedBookings} requested booking${facts.unassignedRequestedBookings === 1 ? '' : 's'} need a staff assignment.` }] : []), ...facts.servicesMissingQualifiedStaff.map((service) => ({ kind: 'UNQUALIFIED_SERVICE' as const, message: `${service.offeringName} has no qualified active staff at ${service.locationName}.` }))] }
