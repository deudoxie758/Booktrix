...
export function BookingFlow({ initialState }: { initialState: CheckoutState }) {
  ...
  const reserve = async (slot: { start: string; segments: Array<Record<string, unknown>> }) => {
    const response = await fetch('/api/booking-holds', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ businessId: initialState.businessId, locationId, checkoutIdentity: crypto.randomUUID(), idempotencyKey: crypto.randomUUID(), segments: slot.segments.map((segment, index) => ({ ...segment, locationId, capacity: 1, priceCents: offerings[index]?.priceCents ?? 0 })) }) })
    const body = await response.json()
    if (response.ok) setHold({ token: body.token, expiresAt: body.expiresAt, expired: false })
  }
  const confirm = async () => {
    if (!hold || !payment || submitting) return
    setSubmitting(true)
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[1fr_22rem]"><section className="rounded-3xl border border-sand-200 bg-cream-50 p-5 sm:p-7">
      ...
      {step === 2 && <><h2 className="mb-5 font-display text-3xl text-cocoa-950">Choose a date and time</h2><AvailabilityPicker slots={slots} loading={loadingSlots} onDate={loadDate} onSelect={reserve} />{hold && !hold.expired && <p role="status" className="mt-4 rounded-2xl bg-clay-100 p-4 text-sm font-semibold text-cocoa-900">Time reserved for 10 minutes.</p>}<button disabled={!date || !hold || hold.expired} onClick={() => setStep(3)} className="mt-6 rounded-full bg-cocoa-900 px-6 py-3 text-sm font-semibold text-white disabled:opacity-40">Continue to details</button></>}
      ...
      {step === 5 && <><h2 className="font-display text-3xl text-cocoa-950">Review and confirm</h2><p className="mt-2 text-cocoa-600">Your slot is reserved while checkout completes.</p>{orderId ? <Link href={`/profile/bookings/${orderId}`} className="mt-6 inline-flex rounded-full bg-cocoa-900 px-6 py-3 text-sm font-semibold text-white">View your booking</Link> : <button disabled={submitting} onClick={confirm} className="mt-6 rounded-full bg-cocoa-900 px-6 py-3 text-sm font-semibold text-white disabled:opacity-40">{submitting ? 'Confirming…' : 'Confirm booking'}</button>}</>}
