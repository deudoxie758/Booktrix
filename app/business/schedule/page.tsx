...
export default async function SchedulePage() {
	const context = await requireWorkspaceRole(['OWNER', 'MANAGER', 'STAFF'])

	if (context.membership.role === 'STAFF') {
		const [schedules, timeOff] = await Promise.all([
			prisma.staffSchedule.findMany({
				where: { membershipId: context.membership.id },
		...
		return <div className="space-y-8"><header><p className="text-xs font-bold uppercase tracking-[.18em] text-clay-600">Availability</p><h1 className="mt-2 font-display text-4xl text-cocoa-950">My schedule</h1><p className="mt-2 text-cocoa-600">Review your recurring hours and time off. Contact your manager to request changes.</p></header><StaffScheduleReadOnly schedules={schedules} timeOff={timeOff} /></div>
	}

	const members = await prisma.businessMembership.findMany({ where: { businessId: context.business.id, active: true, role: { in: ['OWNER', 'MANAGER', 'STAFF'] } }, include: { user: true } })
	const locations = context.availableLocations.map(({ id, name }) => ({ id, name }))
	const staff = members.map(({ id, user }) => ({ id, name: user.name ?? user.email ?? 'Team member' }))
