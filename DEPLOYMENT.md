...
Two of those five legacy changes (the `AuditLog` table and `Booking.createdByRole`) are now captured as an idempotent, forward-only migration (`20260818140000_baseline_legacy_auditlog`), verified to be a no-op against a database that already has them and to correctly create them against one that does not. This makes `npx prisma migrate deploy` self-sufficient from a genuinely empty database — required for disaster recovery and for standing up any new environment (local, CI, staging) without a legacy dump. The production database itself needs no action: this migration applies as part of the normal `migrate deploy` step and does nothing there, since both objects already exist.
