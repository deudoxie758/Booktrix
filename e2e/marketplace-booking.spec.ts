...
test('customer signs in from a held single-service CASH checkout and opens the booking detail', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('Time reserved for 10 minutes.')).toBeVisible({ timeout: 60_000 })
  ...
  const cashPayment = page.getByRole('radio', { name: /pay cash/i })
  await expect(cashPayment).toBeEnabled()
  await cashPayment.check()
  const reviewBooking = page.getByRole('button', { name: /review booking/i })
  await expect(reviewBooking).toBeEnabled()
  await reviewBooking.click()
